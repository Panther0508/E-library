...
export default function EngineerVaultApp() {
  ...
  // ------------------------
  // Utility: safe email input
  // ------------------------
  function getEmail(): string | null {
    const input = window.prompt('Enter your email:');
    if (!input || !input.includes('@')) return null; // basic validation
    return input;
  }
  ...
  async function handleLogin() {
    ...
    if (!email) return alert('Valid email is required!');
  ...
  async function handleSignup() {
    ...
    try {
      const result = await supabase.auth.signUp({ email });
      if (result.error) throw result.error;
  ...
  async function searchBooks(q: string) {
    ...
    try {
      ...
      const mapped = data.docs.slice(0, 10).map((doc: any) => ({
        ...
        pdf: doc.ia ? `https://archive.org/download/${doc.ia}/${doc.ia}.pdf` : null,
      }));
      setBooks(mapped);
    } catch {
      setBooks([]);
      ...
      {entered && (
        <section className="px-6 py-12 max-w-5xl mx-auto">
          ...
          <section className="mt-12">
            <h2 className="text-2xl font-bold text-gold mb-4">Code Snippet Example</h2>
            <SyntaxHighlighter language="cpp" style={oneDark} customStyle={{ borderRadius: 8, backgroundColor: '#000' }}>
              {`#include <iostream>
int main() {
  std::cout << "Hello, EngineerVault!" << std::endl;
}`}
            </SyntaxHighlighter>
          </section>
        </section>
